...
public class Reply extends Marshallable {
	...
	if (type != REPLY_VALUE)
	    throw new UnsupportedOperationException(
		    "Acknowledges do not have return values");
	return returnValue;
    }
	...
	if (bytes.length != sizeof())
	    throw new IllegalArgumentException("byte array shall have size "
		    + sizeof());
	type = bytes[0];
	returnValue = ((bytes[1] & 0xff) << 24) | ((bytes[2] & 0xff) << 16)
	...
	return new byte[] { type, (byte) (returnValue >>> 24),
		(byte) (returnValue >>> 16), (byte) (returnValue >>> 8),
		(byte) returnValue };
    }
